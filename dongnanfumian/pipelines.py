...
class DongnanfumianPipeline(object):
    buffer_list = None
    filename = None
    ...
    def close_spider(self, spider):
        ...
        if self.buffer_list:
            with open('{path}/{cfn}'.format(cfn='{}.txt'.format(self.filename), path=settings.get('STORE_PATH')), 'a+',
                      encoding='utf-8') as f:
    ...
    def process_item(self, item):
        str = '《Root》《S0》{id}《/S0》《S1》{url}《/S1》《S2》{pf}《/S2》《S3a》{vt}《/S3a》《S3b》{sw}《S3b》《S5》{ct}《/S5》《S6》{pt}《/S6》《S9》{level}《/S9》《ID》{cid}《/ID》《S12》{cc}《/S12》《S13》{like}《/S13》《G0》{aid}《/G0》《Q1》{content}《/Q1》《H1》{puser}《/H1》《/Root》\n'.format(
            ...
            ct=item.setdefault('crawlTime', ''), pt=item.setdefault('publishTime', ''),
            level=item.setdefault('level', ''),
            cid=item.setdefault('commentID', ''), like=item.setdefault('like', ''),
            puser=item.setdefault('puserid', ''),
            aid=item.setdefault('authorID', ''), content=item.setdefault('content', ''),
            cc=item.setdefault('comment_count', ''))
        ...
        if not sys.getsizeof(self.buffer_list) > settings.get('FILE_SIZE'):
            ...
            with open(r'{path}/{cfn}'.format(cfn=r'{}.txt'.format(self.filename), path=settings.get('STORE_PATH')),
